...
trim(str_replace('&nbsp', '', strip_tags($_POST['editor']))) == ''){
  header("Location: form-upload.php?error=isi content blogs");
  exit();
}

if(empty($_POST['title'])){
  header("Location: form-upload.php?error=isi judul");
  exit();
}
if(empty($_POST['genre'])){
  header("Location: form-upload.php?error=isi category blog");
  exit();
}
...
if($img_name == ''){  
  $sql = "INSERT INTO blogs (title, description, id_user, tgl_pembuatan, category) VALUE ('$title','$text', $id_user, '$date', '$category')";
  ...
  // Jika sesuai ukurannya
    ...
    if(in_array($img_ex_lc, $allowed_exs)){
      ...
      $sql = "INSERT INTO blogs (thumbnail, title, description, id_user, tgl_pembuatan, category) 
      VALUE ('$datagambar', '$title','$text', $id_user, '$date', '$category')";
